...
ASSUME
    ...
    /\ (\E snapshot \in SnapshotValues : Len(snapshot) = 1)  \* a length-1 snapshot exists
    /\ (\E snapshot \in SnapshotValues : Len(snapshot) > 1)  \* a longer one exists too
    /\ NoFinal \notin SnapshotValues                    \* sentinel distinct from real data
    /\ Placeholder \notin RowValues                     \* synthetic rows are not
    /\ Blank \notin RowValues                           \* ... confusable with
    /\ OverflowMarker \notin RowValues                  \* ... semantic rows,
...
CellRows == RowValues \cup {Placeholder, Blank, OverflowMarker}     \* what a viewport cell may display
...
SnapshotLengths == {Len(snapshot) : snapshot \in SnapshotValues}  \* set of occurring snapshot lengths
MaxSnapshotLength ==                                    \* L_max: the longest snapshot length
    CHOOSE maximum \in SnapshotLengths :                \* (CHOOSE is fine here: the maximum
        \A length \in SnapshotLengths : length <= maximum  \*  of a finite set is unique)
...
NativeTag(source, i, snapshot, wx) ==                   \* ntg: render at width wx, then tag
    [j \in 1..Len(Render(snapshot, wx)) |->             \* one native row per RENDERED row
...
NativeCells(source, cells, wx) ==                       \* lift screen cells to native rows
    [j \in 1..Len(cells) |->                            \* (used when the emulator itself
        [source |-> source, owner |-> cells[j].owner,   \*  pushes viewport rows into
         row |-> cells[j].row, width |-> wx]]           \*  scrollback, e.g. on resize/exit)

PrefixOf(sequence, count) == [j \in 1..count |-> sequence[j]]  \* first `count` elements
...
CommittedRows(k, finals) ==                             \* C(k): finals of blocks 1..k,
    ...
    ELSE CommittedRows(k - 1, finals) \o Tag(k, finals[k])  \* block (= commit) order
...
RetirementRows(lo, hi, finals, firstEmitted) ==         \* logical retirement batch:
    ...
    ELSE TagSlice(lo, finals[lo], firstEmitted + 1, Len(finals[lo]))  \* its UNstreamed suffix,
         \o TaggedRange(lo + 1, hi, finals)             \* later blocks contribute in full
...
NativeRetirementRows(source, lo, hi, finals, firstEmitted, wx) ==
    ...
    ELSE NativeTagSlice(                                \* the same rows,
         ...
         \o NativeRange(source, lo + 1, hi, finals, wx) \* then full later finals

FinalizedRange(lo, hi) ==                               \* "blocks lo..hi are all Finalized"
    \A i \in lo..hi : phase[i] = "Finalized"            \* (a retirement batch precondition)
...
NewerPresented(ph, finals, emission, wx, i) ==          \* how many presented blocks are
    Cardinality({                                       \* NEWER (higher index) than i --
        j \in Blocks :                                  \* used to privilege recency
...
AllocationStateOK(al, requested, ph, finals, emission, wx, hx) ==  \* A_OK: allocation admissibility
    /\ al \in [Blocks -> 0..H]                          \* painted heights in range
    /\ requested \in [Blocks -> 0..H]                   \* requested heights in range
    /\ \A i \in Blocks :
           IF VisiblePresented(ph, finals, emission, wx, hx, i)
           THEN IF ph[i] = "Active"
                THEN /\ al[i] \in 1..H                  \* visible active: painted >= 1,
                     /\ requested[i] \in 1..H           \* target >= 1 (may differ: animating)
                ELSE /\ al[i] \in 1..H                  \* visible finalized: painted >= 1,
...
CanonicalAllocation(ph, finals, emission, wx, hx) ==    \* kappa: the safe default --
    [i \in Blocks |->                                   \* one row per visible block,
...
CreatedCount == Cardinality({i \in Blocks : phase[i] # "Absent"})  \* gamma: how many blocks exist

PartialHeadExists ==                                    \* PH: the head block (c+1) has
    /\ c < CreatedCount                                 \* been created,
    /\ mode[c + 1] = "AppendOnly"                       \* is append-only,
    /\ phase[c + 1] \in {"Active", "Finalized"}         \* is live,
...
Repeat(value, count) == [j \in 1..count |-> value]      \* value^count as a sequence
...
PresentedCells(i) ==                                    \* all slots, ascending block
    ...
    ELSE (IF alloc[i] = 0 THEN <<>> ELSE Slot(i, PreviewSource(i), alloc[i]))  \* next to the cursor);
         \o PresentedCells(i + 1)                       \* zero-alloc blocks contribute nothing
...
Screen ==                                               \* Q: the whole viewport, top to bottom:
    ...
    \o (IF SummaryRows(phase, final, emitted, width, height) = 1
        ...
        ELSE <<>>)
    \o PresentedCells(1)                                \* then the block slots
...
ReplayRows ==                                           \* R: the full replay frame --
    ...
    ELSE NativeRange("Replay", replayCursor, replayEnd, final, width)  \* committed finals 1..c
         \o (IF replayPartial = 0                       \* re-rendered at the NEW width,
...
ReplayRoom ==                                           \* how many blank rows the
    Cardinality({j \in 1..height : Screen[j] = BlankCell})  \* viewport can absorb scroll-free
...
Prefix(left, right) ==                                  \* left is a prefix of right
    ...
    /\ \A j \in 1..Len(left) : left[j] = right[j]       \*  append-only contract)

NoEarlierQueued(i) == \A j \in 1..(i - 1) : phase[j] # "Queued"  \* FIFO admission guard
...
Init ==
    ...
    /\ phase = [i \in Blocks |-> "Absent"]              \* no block exists
    /\ mode = [i \in Blocks |-> "Undeclared"]           \* no contract chosen
    /\ want = [i \in Blocks |-> <<>>]                   \* empty speculation
    /\ final = [i \in Blocks |-> NoFinal]               \* nothing finalized
    /\ emitted = [i \in Blocks |-> 0]                   \* nothing streamed
    /\ alloc = [i \in Blocks |-> 0]                     \* no slot painted
    /\ target = [i \in Blocks |-> 0]                    \* no slot requested
...
Create(declaration) ==                                  \* a new block is declared
    ...
    /\ declaration \in {"Mutable", "AppendOnly"}        \* contract chosen now, forever
...
Update(i, snapshot) ==                                  \* speculation evolves
    /\ running                                          \* host alive
    /\ ~shutdown                                        \* not during shutdown
    /\ phase[i] \in {"Queued", "Active"}                \* only unfinalized blocks change
...
AppendStable ==                                         \* natural streaming: ONE stable row
    ...
    /\ phase[c + 1] \in {"Active", "Finalized"}         \* and only while live
    ...
    /\ LET next == emitted[c + 1] + 1                   \* index of the row to emit
       ...
       IN /\ history' = history \o TagSlice(c + 1, want[c + 1], next, next)  \* ledger += 1 semantic row
          /\ native' =
                 native
                 \o NativeTagSlice("Append", c + 1, want[c + 1], next, next, width)
...
RetireFailure(batchEnd, count) ==                       \* the SAME write, torn partway:
    ...
    /\ LET rows ==
       ...
       IN /\ count \in 0..Len(rows)                     \* the terminal accepted `count`
          /\ native' = native \o PrefixOf(rows, count)  \* rows: an arbitrary PREFIX --
...
Resize(newWidth, newHeight, resizePolicy, pushed) ==    \* terminal geometry changes
    ...
    /\ newWidth \in WidthValues                         \* new geometry and the
    /\ newHeight \in 0..H                               \* policy for native history
    /\ resizePolicy \in ResizeModes
    /\ newWidth # width \/ newHeight # height           \* an actual change
    /\ pushed \in 0..Len(Screen)                        \* emulator may scroll 0..h top
    ...
    /\ LET widthChanged == newWidth # width
       ...
       IN /\ width' = newWidth                          \* adopt the new geometry
          ...
          /\ native' = IF effectiveMode = "Rebuild"
                        ...
                        ELSE native \o pushedRows       \* else: record what the emulator pushed
...
ReplaySynchronousSuccess ==                             \* the single buffered write lands
    ...
    /\ native' = native \o PrefixOf(ReplayRows, replayCut)  \* exactly `cut` rows scroll into
...
ReplaySynchronousFailure(count) ==                      \* the same write, torn partway
    ...
    /\ count \in 0..replayCut                           \* an arbitrary prefix of the
    /\ native' = native \o PrefixOf(ReplayRows, count)  \* scrolled portion landed
...
GracefulExit(push) ==                                   \* clean exit after full drain
    ...
    /\ push \in 0..1                                    \* optionally scroll one last row
...
DetachExit(push) ==                                     \* abandon ship: exit NOW,
    ...
    /\ push \in 0..1                                    \* same optional final scroll
    /\ push = 0 \/ height > 0
    /\ running' = FALSE                                 \* host stops
    /\ stopReason' = "Detach"
    /\ native' = IF push = 0
...
\* -------------------------------------------------------------------------
RetireSuccessAction == \E batchEnd \in Blocks : RetireSuccess(batchEnd)  \* some batch retires
RetireFailureAction ==                                  \* some batch write fails at
    \E batchEnd \in Blocks :                            \* some prefix length
        \E count \in 0..MaxFailureRows : RetireFailure(batchEnd, count)
ReplaySynchronousFailureAction ==                       \* replay write fails at some
    \E count \in 0..MaxFailureRows : ReplaySynchronousFailure(count)  \* prefix length
...
Next ==
    ...
    ELSE \/ \E declaration \in {"Mutable", "AppendOnly"} : Create(declaration)  \* gate open:
         \/ \E i \in Blocks : Admit(i)                                          \* any protocol
         \/ \E i \in Blocks, snapshot \in SnapshotValues : Update(i, snapshot)  \* step may fire
...
TypeOK ==                                               \* T: every variable in range
    /\ c \in 0..N                                       \* frontier within block ids
    /\ phase \in [Blocks -> Phases]                     \* valid phase per block
    /\ mode \in [Blocks -> BlockModes]                  \* valid mode per block
    /\ want \in [Blocks -> SnapshotValues]              \* speculation from the universe
    /\ final \in [Blocks -> SnapshotValues \cup {NoFinal}]  \* final or the sentinel
    /\ emitted \in [Blocks -> 0..MaxSnapshotLength]     \* emitted counter bounded
    /\ alloc \in [Blocks -> 0..H]                       \* painted heights bounded
    /\ target \in [Blocks -> 0..H]                      \* requested heights bounded
    /\ history \in Seq(TaggedRows)                      \* ledger rows well-formed
    /\ native \in Seq(NativeRows)                       \* native rows well-formed
    /\ width \in WidthValues                            \* geometry in range
    /\ height \in 0..H
    /\ resizes \in 0..MaxResizes                        \* resize budget respected
    /\ epoch \in 0..MaxResizes                          \* epochs only at resizes
    /\ replayMode \in ReplayModes                       \* replay state in range
    /\ replayCursor \in 0..(N + 1)                      \* (loose bound; really 0 or 1)
    /\ replayEnd \in 0..N
    /\ replayPartial \in 0..MaxSnapshotLength
    /\ replayPrepared \in BOOLEAN
    /\ replayCut \in 0..MaxFailureRows                  \* cut bounded by max batch size
    /\ flush \in BOOLEAN
    /\ shutdown \in BOOLEAN
    /\ running \in BOOLEAN
...
LifecycleShape ==                                       \* LS: blocks form three bands --
    ...
    /\ \A i \in 1..c :                                  \* band 1: 1..c
           /\ phase[i] = "Committed"                    \* all committed,
           /\ mode[i] \in {"Mutable", "AppendOnly"}     \* with a declared mode
    /\ \A i \in (c + 1)..CreatedCount :                 \* band 2: live blocks
           /\ phase[i] \in {"Queued", "Active", "Finalized"}
           /\ mode[i] \in {"Mutable", "AppendOnly"}
    /\ \A i \in (CreatedCount + 1)..N :                 \* band 3: not yet created
           /\ phase[i] = "Absent"
           /\ mode[i] = "Undeclared"

SnapshotDiscipline ==                                   \* SD: finals exist exactly for
    \A i \in Blocks :                                   \* finalized/committed blocks,
        IF phase[i] \in {"Finalized", "Committed"}
        THEN /\ final[i] \in SnapshotValues             \* are real snapshots,
...
EmissionDiscipline ==                                   \* ED: streaming is head-only --
    /\ \A i \in Blocks :
           ...
           /\ (emitted[i] > 0 =>
                  ...
                  /\ phase[i] \in {"Active", "Finalized"})  \* streamed rows, and only live
...
NoPrematureHistory ==                                   \* every ledger row is owned by
    \A j \in 1..Len(history) :
        ...
        \/ /\ owner \in 1..c                            \* a committed block, or
...
ScreenCapacity ==                                       \* the screen is exactly right:
    /\ Screen \in Seq(Cells)                            \* well-formed cells,
    /\ Len(Screen) = height                             \* exactly `height` of them,
    /\ \A i \in Blocks :
           Cardinality({j \in 1..height : Screen[j].owner = i}) = alloc[i]  \* each block owns alloc[i] rows,
...
ReplayShape ==                                          \* RS: replay bookkeeping is sane
    ...
    /\ (replayMode # "None" =>                          \* in flight: window is 1..replayEnd
                     /\ replayCursor = 1
           /\ replayEnd \in 0..c                        \* over COMMITTED blocks only,
...
NativeSourceSafety ==                                   \* NSS: provenance never lies --
    \A j \in 1..Len(native) :
        ...
        /\ (native[j].source = "Retire" =>              \* Retire rows: from blocks that
               /\ owner \in 1..c                        \* really are committed
               /\ phase[owner] = "Committed")
        /\ (native[j].source \in {"Append", "Replay"} =>  \* streamed/replayed rows: from
               /\ owner \in Blocks                        \* committed blocks or the
               /\ (\/ owner \in 1..c                      \* append-only head -- never
...
AppendOnlyPrefixStep ==                                 \* the append-only contract as
    \A i \in Blocks :                                   \* an action property:
...
QueuedDemand == \E i \in Blocks : phase[i] = "Queued"   \* someone is waiting for space
QueuedPressureRetirement ==                             \* pressure + queued demand
    \A i \in Blocks :                                   \* eventually sweeps a finalized
