...
fn directive_field(name: &str) -> Option<ConstraintField> {
...
fn all_mode_for(name: &str) -> Option<AllMode> {
...
fn push_term(
	...
	if *or_pending && *last_was_term {
		if let Some(previous) = query.terms.last_mut() {
