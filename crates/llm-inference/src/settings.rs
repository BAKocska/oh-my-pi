...
use omp_core::Str;
use omp_llm_catalog::{ModelKey, ProviderId, settings::FallbackChains};
use omp_settings::{
...
pub struct InferenceSettings {
	...
	pub model:     omp_llm_catalog::settings::ModelSettings,
}
	...
	pub fn apply_call(&self, call: &mut crate::Call) {
		...
		let service_tier = call.execution.as_ref().and_then(|execution| {
			self.model.service_tier_for_route(
				...
				omp_llm_catalog::TierAudience::Session,
				None,
			)
		});
		if let OperationCall::Chat(chat) = &mut call.operation {
			...
			if matches!(chat.cache_retention, Setting::Unset) {
				chat.cache_retention = match self.model.cache_retention {
					omp_llm_catalog::settings::CacheRetentionSetting::Auto => Setting::Unset,
					omp_llm_catalog::settings::CacheRetentionSetting::None => {
						Setting::Require(CacheRetention::Request)
					},
					omp_llm_catalog::settings::CacheRetentionSetting::Short => {
						Setting::Prefer(CacheRetention::Short)
					},
					omp_llm_catalog::settings::CacheRetentionSetting::Long => {
