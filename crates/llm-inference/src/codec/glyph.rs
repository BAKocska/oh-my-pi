...
const NOTICE: &str = "<glyph-tokens>\nSome content in this session contains icon characters \
	(private-use Unicode, e.g. nerd font glyphs) that cannot be shown to you directly. Each such \
	character appears as an opaque token like ⟦Ue0a0⟧ — one token is exactly one character. When \
	copying, moving, or preserving surrounding text, copy each token byte-for-byte. Never invent \
	tokens, alter their hex digits, or expand them into anything else. Writing a token in tool \
	arguments writes the original character. To write the literal text ⟦Ue0a0⟧ rather than the \
	character, escape it as ⟦E⟧Ue0a0⟧.\n</glyph-tokens>";
const OPEN: &str = "⟦";
const ESCAPE: &str = "⟦E⟧";

/// Returns whether a canonical operation contains provider-bound glyph text.
pub(crate) fn operation_active(operation: &OperationCall) -> bool {
	let OperationCall::Chat(request) = operation else { return false };
...
pub(crate) fn encode_operation(operation: &OperationCall) -> Option<OperationCall> {
	let OperationCall::Chat(request) = operation else { return None };
...
pub(crate) fn wrap_decoder(inner: DecoderState) -> DecoderState {
...
fn encode_tool(tool: &mut ToolDefinition, notice_added: &mut bool) {
	let Some(description) = tool.description.as_ref() else { return };
...
fn encode_value(value: &Value, notice_added: &mut bool) -> Option<Value> {
	match value {
		Value::String(text) => encode_with_notice(text, notice_added)
...
fn literal_token(text: &str) -> Option<usize> {
	...
	let digits = hex.bytes().take_while(|byte| byte.is_ascii_hexdigit()).count();
	(4..=6)
		.contains(&digits)
		.then(|| hex.get(digits..))
		.flatten()
		.and_then(|tail| tail.starts_with('⟧').then_some(OPEN.len() + 1 + digits + '⟧'.len_utf8()))
}
...
fn decode_text(text: &str) -> Str {
	let Some(first) = text.find(OPEN) else { return Str::new(text) };
	let mut output = String::with_capacity(text.len());
	...
	while cursor < text.len() {
		...
		if let Some(length) = literal_token(tail) {
			...
			if let Some(hex) = token.strip_prefix("⟦U").and_then(|value| value.strip_suffix('⟧'))
...
fn held_suffix_start(text: &str) -> Option<usize> {
	...
	let tail = &text[start..];
	if tail == OPEN || tail.strip_prefix(OPEN).is_some_and(|rest| {
		rest == "E"
			|| rest == "U"
...
mod tests {
	...
	fn glyph_text_round_trips_private_use_planes_and_literal_tokens() {
		...
		assert_eq!(
			encoded,
			"icons: ⟦Ue0a0⟧ ⟦Uf0001⟧ ⟦U100001⟧; literal ⟦E⟧Ue0a0⟧ and ⟦E⟧E⟧"
		);
	...
	fn operation_encoding_is_non_mutating_and_adds_one_notice() {
		...
		assert!(encoded.tools[0].description.as_deref().unwrap().contains(NOTICE));
		assert!(!encoded.messages[0].content.iter().any(|part| matches!(
			part,
			ContentPart::Text { text, .. } if text.contains(NOTICE)
		)));
		let OperationCall::Chat(original) = operation else { unreachable!() };
