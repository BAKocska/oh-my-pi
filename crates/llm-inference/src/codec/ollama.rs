...
fn encode_think<'a>(
	...
	if selection.effort == omp_llm_catalog::ThinkingEffort::Off {
		return Ok(Some(OllamaThink::Disabled(false)));
	}
	let default = match selection.effort {
		omp_llm_catalog::ThinkingEffort::Off => "off",
		omp_llm_catalog::ThinkingEffort::Minimal => "minimal",
		omp_llm_catalog::ThinkingEffort::Low => "low",
		omp_llm_catalog::ThinkingEffort::Medium => "medium",
		omp_llm_catalog::ThinkingEffort::High => "high",
		omp_llm_catalog::ThinkingEffort::XHigh => "xhigh",
		omp_llm_catalog::ThinkingEffort::Max => "max",
	};
	let wire = selection
		.native_effort
		.as_ref()
