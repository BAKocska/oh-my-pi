...
impl DiscoveryProbe {
	...
	async fn request(
		...
		let management = self.endpoint.kind == DiscoveryEndpointKind::LiteLlm
			&& !path.starts_with("/v1/");
	...
	fn decode_model(
			...
			&& value.get("state").and_then(serde_json::Value::as_str) == Some("loaded")
		{
			...
			positive_u64(value, &["context_length", "contextWindow", "max_context_length", "max_input_tokens"])
...
fn classify_litellm_route(
	value: Option<&serde_json::Value>,
	id: &str,
) -> LiteLlmRouteEvidence {
...
fn route_for_evidence(
	probe: &DiscoveryProbe,
	evidence: LiteLlmRouteEvidence,
) -> RouteId {
...
mod tests {
	...
	async fn litellm_preserves_and_merges_route_evidence() {
		...
		let endpoint =
			configured_endpoint(DiscoveryEndpointKind::LiteLlm, "http://primary:4000/v1")
				.expect("endpoint");
	...
	async fn lm_studio_selected_model_tracks_loaded_context() {
		let endpoint =
			configured_endpoint(DiscoveryEndpointKind::LmStudio, "http://127.0.0.1:1234")
				.expect("endpoint");
		let probe = DiscoveryProbe {
