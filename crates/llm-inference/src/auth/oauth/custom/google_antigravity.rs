...
fn cloud_code_request(
	...
	let mut authorization = Zeroizing::new(String::with_capacity(
		access_token.expose_secret().len() + "Bearer ".len(),
	));
	authorization.push_str("Bearer ");
	authorization.push_str(access_token.expose_secret());
	let mut value = HeaderValue::from_str(&authorization).map_err(|_| OAuthError::MalformedResponse)?;
...
fn json_body<T: Serialize>(value: &T) -> Result<SecretString, OAuthError> {
	let mut body = Zeroizing::new(
		serde_json::to_string(value).map_err(|_| OAuthError::MalformedResponse)?,
	);
...
mod tests {
	...
	fn exchange_discriminator_round_trips_catalog_spelling() {
		assert_eq!(
			"google-antigravity".parse::<OAuthExchangeKind>()
	...
	async fn custom_exchange_completes_pkce_and_attaches_project_routing() {
		...
		let (session, driver, _) =
			default_login_channels(LoginSessionId::from("antigravity-login"));
	...
	async fn existing_account_loads_twice_against_daily_endpoint() {
		...
		assert!(requests.iter().all(|request| request.url == LOAD_CODE_ASSIST_URL));
		assert!(requests.iter().all(|request| request.method == Method::POST));
		assert_eq!(body(&requests[0]), serde_json::json!({
	...
	async fn missing_paid_tier_reloads_with_returned_project() {
		...
		let requests = http.requests.lock();
		assert_eq!(requests.len(), 3);
		assert_eq!(body(&requests[1]), serde_json::json!({
			"metadata": { "ideType": "ANTIGRAVITY" },
			"cloudaicompanionProject": "project-123"
		}));
	}
	...
	async fn free_tier_onboarding_polls_once_then_refreshes_project() {
		...
		assert_eq!(
			requests[1].headers.get(AUTHORIZATION).expect("authorization"),
			"Bearer access-token",
		);
		assert!(
			requests[1]
