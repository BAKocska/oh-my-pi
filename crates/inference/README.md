...
`omp-inference` is OMP's typed public contract for model inference. It gives every supported operation—from chat and embeddings through media, realtime, discovery, usage, authentication, and allowlisted native wire access—a concrete request and output type over one closed Tower service envelope.
