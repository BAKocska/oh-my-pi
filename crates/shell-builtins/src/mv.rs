	...
	fn run(self, host: &mut Host) -> i32 {
		...
		if files_len == 1 && !self.matches.contains_id(OPT_TARGET_DIRECTORY) {
			let err = app().error(
				...
				format!(
					"The argument '<{ARG_FILES}>...' requires at least 2 values, but only 1 was \
					 provided"
				),
			);
...
#[allow(
	...
	reason = "destination conflict policy is inherently branch-heavy"
)]
...
fn copy_dir_contents_recursive(
	...
	// Resolve the directory for the read, but rebuild each child path from the
	// display operand directory so recursion and verbose output keep the
	// operand-relative form.
