...
//! commands otherwise have separate argument models and output formats, which
//! is why they are separate modules rather than one with a mode flag.
...
struct RgWalk {
	request: omp_walker::WalkRequest,
	filters: PathFilters,
}
...
impl PathFilters {
	fn includes(&self, path: &Path, file_type: omp_walker::FileType, size: Option<f64>) -> bool {
		let is_dir = file_type == omp_walker::FileType::Dir;
		...
		if file_type != omp_walker::FileType::File {
			return true;
		}
		if !explicitly_included
			&& self
...
fn build_walk(host: &mut Host, cli: &Rg, root: &Path) -> Result<RgWalk, String> {
	...
	let request = omp_walker::WalkRequest::new(root)
		.hidden(include_hidden)
		.gitignore(!no_ignore)
		.skip_git(!no_ignore)
		.skip_node_modules(false)
		.follow_links(omp_walker::FollowLinks::from(cli.follow && !cli.no_follow))
		.detail(if filters.max_filesize.is_some() {
			omp_walker::WalkDetail::Full
		} else {
			omp_walker::WalkDetail::Minimal
		})
		.order(order)
		.emit_root(false)
	...
	let streamed = match walk.request.for_each_entry_with_heartbeat(
		...
		|entry| {
			...
			if !walk.filters.includes(path, entry.file_type, entry.size) {
				return Ok(if entry.file_type == omp_walker::FileType::Dir {
			...
			Ok(if opts.quiet && any_match.get() {
				omp_walker::WalkDecision::Stop
			} else {
				omp_walker::WalkDecision::Include
			})
		},
		|error| {
			had_error.set(true);
