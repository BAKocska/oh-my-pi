...
fn resolve_follow_links(cli: &GrepArgs, matches: &ArgMatches) -> omp_walker::FollowLinks {
	let mut selected = (0, omp_walker::FollowLinks::Roots);
	choose_latest(&mut selected, last_index(matches, "recursive"), omp_walker::FollowLinks::Roots);
	choose_latest(
		&mut selected,
		last_index(matches, "dereference_recursive"),
		omp_walker::FollowLinks::Always,
	);
	if cli.directories == Some(DirectoryAction::Recurse) {
		choose_latest(
			&mut selected,
			last_index(matches, "directories"),
			omp_walker::FollowLinks::Roots,
		);
	}
	choose_latest(
		&mut selected,
	...
	choose_latest(&mut selected, last_index(matches, "no_follow"), omp_walker::FollowLinks::Never);
	choose_latest(&mut selected, last_index(matches, "follow_all"), omp_walker::FollowLinks::Always);
	selected.1
}
...
fn grep_walk_request(
	...
	omp_walker::WalkRequest::new(root)
		...
		.order(omp_walker::WalkOrder::Unordered)
		.emit_root(true)
		.depth(0, usize::MAX)
		...
		.cache(false)
		.filter(omp_walker::WalkFilter::all())
}

/// Recursively search a directory operand while pruning excluded directories.
#[allow(clippy::too_many_arguments, reason = "recursive traversal carries search state")]
fn search_dir<M: Matcher, W: Write>(
	...
	let walk = request.for_each_entry_with_heartbeat(
		...
		|entry: omp_walker::EntryMeta<'_>| {
			...
			if entry.file_type == omp_walker::FileType::Dir {
		...
		|error: omp_walker::DirectoryError<'_>| {
...
#[allow(clippy::too_many_arguments, reason = "top-level search dispatch carries parsed options")]
	...
	impl Read for SnapshottingStdin {
		fn read(&mut self, buf: &mut [u8]) -> io::Result<usize> {
			if self.pos < SNAPSHOT_INPUT.len() {
				let len = buf.len().min(SNAPSHOT_INPUT.len() - self.pos);
				buf[..len].copy_from_slice(&SNAPSHOT_INPUT[self.pos..self.pos + len]);
				self.pos += len;
				return Ok(len);
			}
			if !self.snapped {
