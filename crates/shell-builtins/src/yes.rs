...
/// Expands the single output line to the largest whole-line batch under
/// [`BUF_SIZE`].
...
/// Writes one prepared batch at a time until output fails or the host is
/// cancelled.
...
mod tests {
	...
	fn run_with(
		...
		host.stdout =
			OpenFile::Stream(Box::new(FailingWriter { state: Arc::clone(&state), fail_kind }));
