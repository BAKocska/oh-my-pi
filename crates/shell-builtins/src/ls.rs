...
mod config {
	...
	pub struct Config {
		...
		#[allow(dead_code, reason = "Windows retains the shared configuration shape")]
	...
	impl Config {
		...
		#[allow(
			...
			reason = "option precedence is centralized in this parser"
		)]
		pub(super) fn from(
			options: &clap::ArgMatches,
			host: &Host,
...
mod dired {
	...
	///    - `BytePosition`: Represents a byte range with start and end
	///      positions.
	///    - `DiredOutput`: Contains positions for DIRED and SUBDIRED outputs and
	///      maintains a padding value.
	///
	/// 2. **Padding**:
	///    - Padding is used when dealing with directory names or the "total"
	///      line.
	...
	///      `calculate_and_update_positions` compute byte positions based on
	///      output length, previous positions, and padding.
	///
	/// 4. **Output**:
	///    - The module provides functions to print the DIRED output
	///      (`print_dired_output`) based on calculated positions and
	///      configuration.
	///    - Helpers like `print_positions` print positions with specific
	///      prefixes.
	///
	/// Overall, the module ensures each entry in the DIRED output has the
	/// correct byte position, considering additional lines or padding affecting
	/// positions.
	use super::Config;
...
mod display {
	...
	/// Show the directory name in the case where several arguments are given to
	/// ls
	...
	#[allow(dead_code, reason = "the device variant is retained for platform-specific formatting")]
	enum SizeOrDeviceId {
		Size(String),
	...
	#[allow(
		clippy::cognitive_complexity,
		reason = "all display modes share this rendering entry point"
	)]
			...
			match config.format {
				...
				Format::Commas => {
					...
					if let Some(name) = names.next() {
						...
						current_col = xutf::width_ansi_str(&name.to_string_lossy()) as u16 + 2;
					}
					for name in names {
						let name_width = xutf::width_ansi_str(&name.to_string_lossy()) as u16;
	...
	const GRID_SEPARATOR_SIZE: usize = 2;
	...
	fn compute_grid_dimensions(
		...
		GridDimensions { num_rows, column_widths }
	}
	...
	fn grid_dimensions(
		...
		if widths.len() == 1 {
			return GridDimensions { num_rows: 1, column_widths: vec![widths[0]] };
		}

		let widest = widths.iter().copied().max().unwrap_or(0);
		let widest_column = widest + GRID_SEPARATOR_SIZE;
		if widest_column > terminal_width {
			return GridDimensions { num_rows: widths.len(), column_widths: vec![widest] };
		}

		let min_columns = widths
			.len()
			.min((terminal_width + GRID_SEPARATOR_SIZE) / widest_column);
		let max_rows = widths.len().div_ceil(min_columns);
		let mut dimensions = compute_grid_dimensions(widths, max_rows, min_columns, direction);
		if max_rows == 1 {
			return dimensions;
		}

		// Add columns while their measured content and separators still fit.
		for num_columns in min_columns + 1..=widths.len() {
			let separator_width = (num_columns - 1) * GRID_SEPARATOR_SIZE;
			let Some(content_width) = terminal_width.checked_sub(separator_width) else {
				break;
			};
			let num_rows = widths.len().div_ceil(num_columns);
			let candidate = compute_grid_dimensions(widths, num_rows, num_columns, direction);
			if candidate.column_widths.iter().sum::<usize>() <= content_width {
	...
	fn write_grid<W: Write>(
		...
		let widths: Vec<_> = cells
			.iter()
			.map(|cell| xutf::width_ansi_str(&cell.to_string_lossy()))
			.collect();
		let widest = widths.iter().copied().max().unwrap_or(0);
		let dimensions = grid_dimensions(&widths, terminal_width, direction);
		let padding = vec![b' '; widest + GRID_SEPARATOR_SIZE];

		for row in 0..dimensions.num_rows {
			let mut cursor = 0;
			for column in 0..dimensions.column_widths.len() {
				let (current, offset) = match direction {
					Direction::LeftToRight => (row * dimensions.column_widths.len() + column, 1),
					Direction::TopToBottom => (row + dimensions.num_rows * column, dimensions.num_rows),
				};
				if current >= cells.len() {
					break;
				}

				write_os_str(out, &cells[current])?;
				let last_column = column == dimensions.column_widths.len() - 1;
				if last_column || current + offset >= cells.len() {
					break;
				}

				let cell_width = widths[current];
				let total_spaces = dimensions.column_widths[column] - cell_width + GRID_SEPARATOR_SIZE;
				if tab_size == 0 {
					out.write_all(&padding[..total_spaces])?;
				} else {
					cursor += cell_width;
					...
					if closest_tab > total_spaces {
						...
						let remaining = total_spaces - closest_tab;
						let tabs = 1 + remaining / tab_size;
						for _ in 0..tabs {
							out.write_all(b"\t")?;
						}
						out.write_all(&padding[..remaining % tab_size])?;
					}
					cursor += total_spaces;
				}
			}
	...
	fn display_grid<W: Write>(
		...
		let names: Vec<_> = names
			.map(|name| {
				// Align unquoted names with entries that start with a quote.
				if quoted && !os_str_starts_with(&name, b"'") && !os_str_starts_with(&name, b"\"") {
					let mut padded = OsString::from(" ");
					padded.push(name);
					padded
				} else {
					name
			...
			.collect();
		write_grid(&names, width as usize, direction, tab_size, out)
	}
	...
	/// displaying.
	///
	/// This function relies on the following parameters in the provided
	/// `&Config`:
	/// * `config.quoting_style` to decide how we will escape `name` using
	///   [`locale_aware_escape_name`].
	/// * `config.inode` decides whether to display inode numbers beside names
	///   using [`display_inode`].
	...
	#[allow(
		clippy::cognitive_complexity,
		reason = "name rendering combines independent display features"
	)]
	...
	/// * permissions and `system_time` are already guaranteed to be
	///   pre-formatted in fixed length.
	...
	#[allow(
		...
		reason = "the output format intentionally contains literal padding"
	)]
	#[allow(
		clippy::cognitive_complexity,
		reason = "long-format rendering combines independent columns"
	)]
	fn display_item_long(
		...
		if let Some(md) = item.metadata() {
			...
			let item_display = display_item_name(
				...
				None,
				state.style_manager.as_mut(),
				LazyCell::new(|| xutf::width_ansi_str(&String::from_utf8_lossy(&state.display_buf))),
			);
	...
	mod grid_tests {
		...
		use super::{Direction, write_grid};

		fn render(names: &[&str], width: usize, direction: Direction, tab_size: usize) -> String {
			let names = names
				.iter()
				.map(|name| OsString::from(*name))
				.collect::<Vec<_>>();
			let mut output = Vec::new();
			write_grid(&names, width, direction, tab_size, &mut output).expect("render grid");
			String::from_utf8(output).expect("grid output is UTF-8")
		}

		#[test]
		fn lays_out_odd_entries_in_both_directions() {
			let names = ["one", "two", "three", "four", "five"];
			assert_eq!(
				render(&names, 15, Direction::LeftToRight, 0),
				"one    two\nthree  four\nfive\n"
			);
			assert_eq!(
				render(&names, 15, Direction::TopToBottom, 0),
				"one    four\ntwo    five\nthree\n"
			);
		}

		#[test]
		fn measures_ansi_and_unicode_cells_with_xutf() {
			let names = ["\u{1b}[31m界\u{1b}[0m", "a", "xx", "b"];
			assert_eq!(
				render(&names, 8, Direction::TopToBottom, 0),
				"\u{1b}[31m界\u{1b}[0m  xx\na   b\n"
			);
		}

		#[test]
		fn preserves_tab_filling() {
			let names = [
				"one", "two", "three", "four", "five", "six", "seven", "eight", "nine", "ten",
				"eleven", "twelve",
			];
			assert_eq!(
				render(&names, 24, Direction::LeftToRight, 2),
				"one\t\t two\t\t three\nfour\t five\t\t six\nseven\t eight\t nine\nten\t\t eleven\t \
				 twelve\n"
			);
		}
...
pub fn list(locs: Vec<&Path>, config: &Config, stdout: OpenFile) -> io::Result<()> {
	...
	for (pos, path_data) in dirs.iter().enumerate() {
		...
		state
			.listed_ancestors
			.insert(FileInformation::from_path(&path_data.fs_path, path_data.must_dereference)?);
...
#[allow(unused_variables, reason = "metadata is only used on Unix")]
...
fn file_is_executable(md: &Metadata) -> bool {
	...
	#[allow(
		...
		reason = "platform mode constants have varying integer widths"
	)]
...
fn get_security_context<'a>(
	...
	// 1.
	if must_dereference
		&& let Err(err) = get_metadata_with_deref_opt(&config.runtime.resolve(path), must_dereference)
	{
