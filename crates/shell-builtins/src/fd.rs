...
fn search(
	...
	for search_path in &search_paths {
		...
		let outcome = match request.collect_with_heartbeat(cancel_heartbeat(cancelled)) {
			Ok(outcome) => outcome,
			Err(omp_walker::WalkError::Interrupted(_)) if cancelled.load(Ordering::Relaxed) => break,
		...
		let status = request.for_each_entry_with_heartbeat(
			...
			|entry| {
				if cancelled.load(Ordering::Relaxed) || max_results.is_some_and(|max| matches >= max) {
					return Ok(omp_walker::WalkDecision::Stop);
				}
...
fn process_walker_entry<W: Write>(
	...
	path: &Path,
	depth: usize,
	file_type: omp_walker::FileType,
	out: &mut W,
	matches: &mut usize,
) -> io::Result<omp_walker::WalkDecision> {
	let is_directory = file_type == omp_walker::FileType::Dir;
	if depth == 0 && is_directory {
		return Ok(omp_walker::WalkDecision::Skip);
	}
	if config.excludes.matches(path, &config.base_dir) {
		return Ok(if is_directory {
			omp_walker::WalkDecision::SkipDescend
		} else {
			omp_walker::WalkDecision::Skip
		});
	}
	if is_directory {
		if ignore_contains.iter().any(|name| path.join(name).exists()) {
			return Ok(omp_walker::WalkDecision::SkipDescend);
		}
		if config.prune
			&& config
				.matcher
	...
	*matches = (*matches).saturating_add(1);
	if config.quiet {
		return Ok(omp_walker::WalkDecision::Include);
	}
...
fn matches_walker_type_filter(
	...
	let kind_matches = if filter.has_kind() {
		(filter.regular && file_type == omp_walker::FileType::File)
			|| (filter.directory && file_type == omp_walker::FileType::Dir)
			|| (filter.symlink && file_type == omp_walker::FileType::Symlink)
	} else {
		true
	};
	if !kind_matches {
...
/// `omp_walker`'s per-entry heartbeat never checks the flag and a cancelled
/// walk keeps traversing until the whole tree is collected.
/// Returning [`io::ErrorKind::Interrupted`] surfaces as
/// [`omp_walker::WalkError::Interrupted`], which the callers translate to a
...
fn walker_error_to_io(err: omp_walker::WalkError<io::Error>) -> io::Error {
	match err {
		omp_walker::WalkError::Interrupted(err) => err,
		omp_walker::WalkError::InvalidData { path, message } => {
			io::Error::other(format!("{}: {message}", path.display()))
		},
	}
}

fn walker_collect_error_to_io(err: omp_walker::WalkError<String>) -> io::Error {
	match err {
		omp_walker::WalkError::Interrupted(err) => io::Error::other(err),
		omp_walker::WalkError::InvalidData { path, message } => {
			io::Error::other(format!("{}: {message}", path.display()))
		},
	}
...
fn process_collected_entry<W: Write>(
	...
	let is_directory = entry.file_type == omp_walker::FileType::Dir;
	if depth == 0 && is_directory {
		return Ok(());
	}
...
mod tests {
	...
	fn cancel_heartbeat_aborts_collect_with_heartbeat() {
		...
		assert!(
			matches!(err, omp_walker::WalkError::Interrupted(_)),
