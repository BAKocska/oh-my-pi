...
pub mod command {
	...
	use crate::sed::{
		error_handling::{ScriptLocation, SedResult, runtime_error},
	...
	#[derive(Clone, Debug)]
	/// Elements that shall be appended at the end of each command processing
	/// cycle
...
pub mod compiler {
	...
	use crate::sed::{
		...
		error_handling::{ScriptLocation, SedError, SedResult, compilation_error, semantic_error},
		fast_regex::Regex,
		named_writer::NamedWriter,
		script_char_provider::ScriptCharProvider,
	...
	/// Compile the provided regular expression string into a corresponding
	/// engine. An empty pattern results in None, which means that the last RE
	/// employed at runtime will be used.
	fn compile_regex(
		lines: &ScriptLineProvider,
	...
	fn compile_subst_command(
		...
		subst.regex =
			compile_regex(lines, line, &pattern, context, subst.ignore_case, subst.multiline)?;
	...
	mod tests {
		...
		fn test_compile_subst_flag_g_and_number_should_fail() {
			...
			let err =
				compile_subst_flags(&lines, &mut chars, &mut subst, false, false, None).unwrap_err();
			assert!(
				err.to_string()
...
pub mod delimited_parser {
	...
	use crate::sed::{
		...
		error_handling::{SedResult, compilation_error},
		script_char_provider::ScriptCharProvider,
		script_line_provider::ScriptLineProvider,
	};
...
pub mod fast_io {
	...
	impl<'a> IOChunk<'a> {
			...
			match &self.content {
				...
				IOChunkContent::MmapInput { content, .. } => {
					if self.utf8_verified.get() {
						// SAFETY: `utf8_verified` is set only after `from_utf8` accepts this immutable
						// mapping.
		...
		pub fn fields_mut(&mut self) -> Result<(&mut String, &mut bool), SedError> {
			...
			match &mut self.content {
				...
				#[allow(unreachable_patterns, reason = "ensure_owned guarantees the owned variant")]
	...
	/// Unified reader that uses mmap when possible, falls back to buffered
	/// reading.
	...
	impl<'a> LineReader<'a> {
		...
		pub fn open_with_host(path: &PathBuf, host: &mut Host) -> io::Result<Self> {
			...
			#[cfg(unix)]
			{
				// SAFETY: `file` remains owned alongside the mapping for the mapping's entire
				// lifetime.
	...
	impl OutputBuffer {
			...
			match &new_chunk.content {
				IOChunkContent::MmapInput { full_span, .. } => {
					...
					let (flush_action, reset) = if let Some(old_chunk) = self.mmap_chunk.as_mut() {
						...
						// SAFETY: both pointers originate from the same live memory-mapped region.
		...
		fn flush_mmap(&mut self, _cover: WriteRange) -> io::Result<()> {
			if let Some(chunk) = self.mmap_chunk.as_mut() {
				...
				chunk.len -= written;
				// SAFETY: advancing within the just-written mapped range preserves a valid
				// pointer.
...
pub mod fast_regex {
	...
	use crate::sed::{
		error_handling::{SedError, SedResult},
		fast_io::IOChunk,
	};
	...
	impl LiteralMatcher {
		...
		pub fn iter<'t>(&'t self, haystack: &'t [u8]) -> LiteralMatches<'t> {
			let nlen = self.needle.len();
			match self.match_type {
				AnchoredMatch::Both | AnchoredMatch::Begin | AnchoredMatch::End => {
					LiteralMatches::One(self.find(haystack).into_iter())
				},
				AnchoredMatch::Free => LiteralMatches::Free {
					iter: memmem::find_iter(haystack, &self.needle),
					haystack,
					nlen,
				},
			}
	...
	impl<'t> Iterator for LiteralMatches<'t> {
		type Item = (usize, usize, &'t str);
		...
		pub fn captures_iter<'t>(&'t self, chunk: &'t IOChunk) -> SedResult<CaptureMatches<'t>> {
			match self {
				Regex::Literal(m) => {
					...
					Ok(CaptureMatches::Literal(LiteralCaptureMatches { iter: m.iter(haystack) }))
				},
	...
	/// Allocation-free adapter from literal matches to capture results.
	pub struct LiteralCaptureMatches<'t> {
		iter: LiteralMatches<'t>,
	}

	impl<'t> Iterator for LiteralCaptureMatches<'t> {
	...
	/// Unified enum for holding literal, byte, or fancy capture iterators.
...
pub mod in_place {
	...
	impl InPlace {
		...
		/// Creates an editing engine backed by an in-memory output for unit
		/// tests.
		#[cfg(test)]
...
pub mod processor {
	...
	use crate::{
		...
		sed::{
			...
			error_handling::{IoContext, ScriptLocation, SedResult, input_runtime_error},
	...
	#[allow(dead_code, reason = "retained as an address-application primitive")]
	...
	#[allow(
		...
		reason = "sed execution follows the command language directly"
	)]
...
#[allow(
	clippy::cognitive_complexity,
	reason = "CLI construction mirrors sed's broad option surface"
)]
fn uu_app() -> Command {
	let util_name = "sed";
