...
/// Selects the processes matching `options`, and resolves the host chain from
/// the same process-table snapshot.
///
/// Both come from one `ProcInfo::all()`: `pkill` needs the chain to decide what
/// it may signal, and taking a second snapshot for it would walk the whole
/// table again.
fn select_processes(
