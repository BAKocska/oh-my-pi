...
//! process-global signal disposition for `-i` and SIGPIPE. An in-process
//! builtin cannot do that safely: `-i` is accepted without changing the shell's
//! signal policy, while `BrokenPipe` from the invocation's stdout is handled
//! according to `--output-error` and does not prevent writes to the remaining
//! outputs.
