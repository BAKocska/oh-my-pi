...
/// Borrowed regular-file candidate delivered by streaming walk APIs.
#[derive(Clone, Copy, Debug, PartialEq)]
pub struct FileCandidateRef<'a> {
	/// Absolute filesystem path to the regular file.
	pub path:     &'a Path,
	/// Relative path from the walk root, using `/` separators.
	pub relative: &'a str,
...
impl WalkRequest {
	...
	///
	/// Candidate paths and relative names borrow per-worker scratch and remain
		...
		HE: Display,
	{
		let mut outcome = self.collect_with_heartbeat(heartbeat)?;
		outcome.entries.retain(CollectedEntry::is_file);
		parallel_for_each_init(&outcome.entries, PathBuf::new, |path, entry| {
			path.clear();
			path.push(&self.root);
			if !entry.path.is_empty() {
				path.push(&entry.path);
			}
			operation(FileCandidateRef {
				path:     path.as_path(),
				relative: &entry.path,
				mtime:    entry.mtime,
				size:     entry.size,
		...
		.map_err(|err| interrupted(err.to_string()))?;
		Ok(outcome.stats)
	}
	...
	/// Candidate paths and relative names borrow worker-local traversal scratch
	...
	S: for<'candidate> Fn(FileCandidateRef<'candidate>) -> Result<ParallelWalkControl, E> + Sync,
{
	...
	fn visit_pre_decided(&mut self, entry: Entry<'_>) -> Result<WalkControl, Self::Error> {
		...
		let candidate = FileCandidateRef {
			path:     entry.path,
...
fn interrupted<E>(error: E) -> WalkError<E> {
...
impl<H> WalkContext<'_, H> {
		...
		H: FnMut() -> Result<(), V::Error>,
	{
		...
		let decision = visitor.decide_pre_descend(&meta).map_err(interrupted)?;
		if decision.stop {
		...
		H: FnMut() -> Result<(), V::Error>,
	{
		...
		let decision = {
			...
			visitor.decide_pre_descend(&meta).map_err(interrupted)?
		};
		if decision.stop {
			return Ok(true);
		}
...
mod platform {
	...
	const NAME_BUFFER_SIZE: usize = libc::NAME_MAX as usize + 1;
	...
	fn stat_entry(
		...
		if name.len() >= NAME_BUFFER_SIZE {
			return Err(io::Error::new(io::ErrorKind::InvalidData, "entry name exceeds NAME_MAX"));
		}
		let mut name_buffer = [0; NAME_BUFFER_SIZE];
		name_buffer[..name.len()].copy_from_slice(name);
		let name = CStr::from_bytes_with_nul(&name_buffer[..=name.len()])
			.expect("trimmed directory entry name has one trailing NUL");
		...
		F: FnMut(RawDirEntry<'_>) -> Result<ReadDirControl, WalkError<E>>,
	{
		...
		loop {
			...
			loop {
				...
				if name_len % 2 != 0
					|| name_unit_len > MAX_FILE_NAME_UNITS
					|| name_offset + name_len > buffer.len()
				{
					return Err(invalid_data("invalid NtQueryDirectoryFile name length").into());
				}
				let mut name_units = [0; MAX_FILE_NAME_UNITS];
				for (unit, chunk) in name_units[..name_unit_len]
					.iter_mut()
					.zip(buffer[name_offset..name_offset + name_len].chunks_exact(2))
				{
					*unit = u16::from_ne_bytes([chunk[0], chunk[1]]);
				}
				let name = OsString::from_wide(&name_units[..name_unit_len]);
