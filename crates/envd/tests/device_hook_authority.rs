...
fn dynamic_device_catalog_transitions_are_atomic_and_generation_fenced() {
	...
	catalog
		.replace(
			owner.clone(),
			LeafVersion { manager_generation: 7, definition_epoch: 1 },
			vec![leaf("dynamic/echo"), leaf("dynamic/search")],
		)
		.expect("sealed mount");
	let epoch = catalog
	...
	assert_eq!(
		catalog
			.set_availability_many(6, &[(
				owner,
				AvailabilityDelta { name: sf!("dynamic/echo"), mounted: true, reason: None },
			)])
...
async fn queued_hook_callback_cancellation_is_exact() {
	...
	let (ready, _running) = router.dispatch(sf!("acme/hooks"), request(1)).expect("running");
	assert!(ready.is_some());
	let (ready, queued) = router.dispatch(sf!("acme/hooks"), request(2)).expect("queued");
	assert!(ready.is_none());
	assert!(router.cancel_queued("acme/hooks", 2).expect("cancel queued"));
