...
) -> Result<PowerSettings, crate::settings::manager::SettingsManagerError> {
	let project = std::env::current_dir().ok();
	let manager = crate::settings::manager::SettingsManager::open(
		crate::settings::manager::SettingsPaths::discover(data_dir, project.as_deref()),
	)?;
	let projection = manager
		.snapshot()
		.project::<PowerSettings>()
		.map_err(|source| crate::settings::manager::SettingsManagerError::Projection { source })?;
