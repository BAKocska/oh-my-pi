...
impl HtmlThemePalette {
	...
	pub fn from_theme(theme: Theme) -> Self {
		Self {
			...
			background: css_color(theme.panel, "#0c0f12"),
			card:       css_color(theme.surface, "#3a3f4b"),
			border:     css_color(theme.border, "#454b58"),
			accent:     css_color(theme.accent, "#61afef"),
			muted:      css_color(theme.muted, "#5c6370"),
			error:      css_color(theme.err, "#e06c75"),
		}
	}
}
...
pub fn render_html(tree: &SessionTree) -> Result<String, ExportError> {
	render_html_with_theme(tree, Theme::default())
}

/// Renders a self-contained viewer using the active semantic TUI theme.
pub fn render_html_with_theme(tree: &SessionTree, theme: Theme) -> Result<String, ExportError> {
	let model = serde_json::to_string(tree)?.replace('<', "\\u003c");
	let palette = HtmlThemePalette::from_theme(theme);
...
fn css_color(color: Color, fallback: &str) -> String {
	match color {
		...
		Color::Default => fallback.to_owned(),
	}
}
...
fn indexed_rgb(index: u8) -> (u8, u8, u8) {
	...
	if index >= 232 {
		let level = 8_u8.saturating_add(index.saturating_sub(232).saturating_mul(10));
