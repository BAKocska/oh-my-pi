...
use crate::rpc_mode::HostResourceBroker;

static BROKER: RwLock<Weak<HostResourceBroker>> = RwLock::new(Weak::new());

/// Installs the sole live RPC host-resource authority.
pub(crate) fn bind(broker: &Arc<HostResourceBroker>) -> Result<(), ()> {
...
pub(crate) fn unbind(broker: &Arc<HostResourceBroker>) {
	let mut current = BROKER.write();
	if current
		.upgrade()
		.as_ref()
...
impl HostUriResolver {
	pub(crate) fn new() -> Self {
		Self { lines: LineOffsetCache::default() }
	}
}
...
impl Resolve for HostUriResolver {
	async fn read<'a>(
		...
		let broker = BROKER.read().upgrade().ok_or_else(|| Fault::Unsupported {
