...
impl PresenceRegistry {
	...
	pub(crate) fn register(
		...
		crate::settings::io::atomic_replace(&path, &encoded)?;
	...
	fn expire_stale_locked(
		&self,
		leases: &mut HashMap<Str, PathBuf>,
...
impl PresenceLease {
	pub(crate) fn id(&self) -> &Str {
		self.id.as_ref().expect("an active presence lease has an id")
	}
...
mod tests {
	...
	fn two_live_clients_coexist() {
		...
		assert!(records.iter().any(|record| record.client_id == sf!("client-a")));
		assert!(records.iter().any(|record| record.client_id == sf!("client-b")));
	}
}
