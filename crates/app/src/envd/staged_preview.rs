...
use omp_tools::staging::{
	PREVIEW_PENDING_NOTICE, PendingPreview, PreviewDecision, PreviewObserver,
	PreviewObserverError, PreviewRejection, parse_resolution_invoke,
};
...
mod tests {
	...
	use omp_agent::{
		CampaignMachine as _, CampaignStack, EngageOptions, Verdict,
	...
	async fn stage_forces_dyn_resolution_and_resolution_disengages() {
		...
		choices.register_pending_invoker(
			pending.id.clone(),
			pending.source_tool.clone(),
			invoker,
		);
		...
		let receipt = campaigns
			.engage(
				spec(),
				Box::new(StagedPreviewCampaign::new(pending.clone())),
		...
		assert!(matches!(
			choices.claim_next(),
			Some(omp_llm_inference::call::ToolChoice::Named(tool)) if tool == "dyn"
		));
		let outcome = choices
		...
		assert_eq!(
			decisions.lock().as_slice(),
			&[PreviewDecision::Resolve {
