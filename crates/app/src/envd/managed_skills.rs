...
use crate::{
	discovery::{managed_skills, skills::SkillDiscoverySettings},
	skills::{SkillSnapshot, managed::ManagedSkillCandidate},
};
...
pub struct ManagedSkills {
	...
	revision:       AtomicU64,
	inventory:      RwLock<Arc<SkillSnapshot>>,
...
impl ManagedSkills {
	...
	pub fn new(root: PathBuf, authored_names: BTreeSet<Str>) -> Self {
		Self {
			...
			inventory: RwLock::new(Arc::new(SkillSnapshot::default())),
	...
	fn serialize_name<'a>(&self, raw: &'a str) -> Result<(Str, NameLock<'_>), AuthorityError> {
		...
		if !crate::skills::managed::is_valid_name(&normalized) {
	...
	fn candidate(
		...
		ManagedSkillCandidate::new(request.name, description, body).map_err(|error| match error {
			crate::skills::managed::CandidateError::InvalidName => AuthorityError::InvalidName,
			crate::skills::managed::CandidateError::InvalidDescription => {
				AuthorityError::InvalidDescription
			},
			crate::skills::managed::CandidateError::EmptyBody => AuthorityError::EmptyBody,
			crate::skills::managed::CandidateError::TooLarge => AuthorityError::TooLarge,
		})
	}
	...
	fn refresh(&self) {
		let agent_root = self.root.parent().unwrap_or(&self.root);
		let discovered =
			managed_skills::discover_dead_last(agent_root, &SkillDiscoverySettings::default());
		*self.inventory.write() =
			Arc::new(SkillSnapshot::from_declarations(&discovered.declarations));
	}
}
...
mod tests {
	...
	fn serializes_mutations_and_refuses_links_and_authored_shadows() {
		...
		assert!(authority.inventory().get("new-skill").is_some());
	}
}
