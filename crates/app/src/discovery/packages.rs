...
use crate::{discovery::cache::DiscoveryCache, ext::lock::InstalledRecord};
...
pub fn invalidate_after_transaction(
	...
	committed: Result<&[Str], &crate::ext::ExtensionError>,
) -> usize {
...
mod tests {
	...
	fn explicit_only_and_realpath_dedup_are_enforced() {
		...
		let installed = InstalledRecord {
			version:    1,
			extensions: vec![crate::ext::lock::InstalledExtension {
