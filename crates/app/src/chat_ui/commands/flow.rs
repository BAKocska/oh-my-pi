...
command!(todo, 452, "todo", icon: Todo, [], "Inspect or update session tasks", [Session], false, raw("[subcommand]", ["show", "edit", "copy", "expand", "collapse", "export", "import", "append", "start", "done", "drop", "rm", "help"]) => |host, args| host.todo(args));
...
command!(goal, 470, "goal", icon: Goal, ["guided-goal"], "Start or control a guided goal", [Execution], false, raw("[set|pause|resume|complete|drop|budget|status|stop]", ["set", "pause", "resume", "complete", "drop", "budget", "status", "stop"]) => |host, args| host.guided_goal(args));
