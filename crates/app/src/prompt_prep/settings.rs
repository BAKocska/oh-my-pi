...
impl PromptSettings {
	...
	pub fn with_cli(mut self, cli: &crate::cli::PromptArgs) -> Self {
		...
		if let Some(value) = cli.include_model_in_prompt {
			self.include_model_in_prompt = value;
		}
		if let Some(value) = cli.include_workstation {
			self.include_workstation = value;
		}
		if let Some(value) = cli.include_workspace_tree {
			self.include_workspace_tree = value;
		}
		if let Some(value) = cli.render_mermaid {
			self.render_mermaid = value;
		}
		if let Some(value) = cli.skills_enabled {
			self.skills_enabled = value;
		}
		if let Some(value) = &cli.custom_prompt {
			self.custom_prompt = Some(value.clone());
		}
		if let Some(value) = &cli.append_prompt {
...
mod tests {
	...
	fn cli_overrides_freeze_without_changing_pi_defaults() {
		...
		let settings = PromptSettings::default().with_cli(&cli);
