...
/// 	omp_shell_engine::builtins::parse_known::<CommandLineArgs, _>(std::env::args());
/// if raw_args.is_some() {
...
#[allow(
	...
	reason = "the declaration command contract is one cohesive paragraph"
)]
/// Returns a built-in command registration, given an implementation of the
...
#[expect(clippy::unused_async, reason = "the callback ABI requires an async implementation")]
