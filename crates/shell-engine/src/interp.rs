...
impl ExecutionParameters {
	...
	+ std::iter::FusedIterator {
		...
		#[allow(
			...
			reason = "the returned iterator must own merged descriptor data"
		)]
...
pub trait Execute {
	...
	) -> impl Future<Output = Result<ExecutionResult, error::Error>> + Send;
}
...
trait ExecuteInPipeline<SE: extensions::ShellExtensions> {
	fn execute_in_pipeline(
		...
		params: ExecutionParameters,
	) -> impl Future<Output = Result<ExecutionSpawnResult, error::Error>> + Send;
}
...
impl Execute for ast::CompoundList {
	async fn execute<SE: extensions::ShellExtensions>(
		...
		execute_compound_list(self, shell, params).await
	}
}

/// Executes a compound list behind one dyn-erased allocation.
///
/// Every recursive AST construct (function bodies, subshells, `if`/`while`
/// bodies, case items) funnels back through [`ast::CompoundList`], so fully
/// unboxed [`Execute`] futures would form an opaque-type cycle. The concrete
/// boxed signature cuts that cycle at its single chokepoint; the allocation
/// is once per compound list, dominated by the commands it runs.
fn execute_compound_list<'a, SE: extensions::ShellExtensions>(
	list: &'a ast::CompoundList,
	shell: &'a mut Shell<SE>,
	params: &'a ExecutionParameters,
) -> Pin<Box<dyn Future<Output = Result<ExecutionResult, error::Error>> + Send + 'a>> {
	Box::pin(async move {
		ensure_not_cancelled(params)?;
		let mut result = ExecutionResult::success();
	...
	async fn execute_in_pipeline(
		...
		match self {
			...
			Self::Compound(compound, redirects) => {
				...
				// Multi-stage pipelines run each stage in an owned subshell.
				// Spawn compound stages before moving on to downstream stages:
				// inline execution can fill the connecting pipe before its
				// reader exists and deadlock the pipeline.
			...
			Self::ExtendedTest(e, redirects) => {
				...
				// Evaluate the extended test expression. Bash reserves status 2 for an
				// invalid regular expression instead of treating it as a non-match.
				let result = match extendedtests::eval_extended_test_expr(
					&e.expr,
				...
				.await
				{
					Ok(true) => 0,
					Ok(false) => 1,
					Err(error) if matches!(error.kind(), error::ErrorKind::InvalidRegexError(..)) => 2,
...
impl Execute for ast::CoprocessCommand {
	async fn execute<SE: extensions::ShellExtensions>(
		...
		execute_coprocess(self, shell, params).await
	}
}

/// Executes a coprocess behind one dyn-erased allocation.
///
/// The coprocess body re-enters command execution (`Command` →
/// `CompoundCommand` → coprocess), which would otherwise close an
/// opaque-type cycle among the unboxed [`Execute`] futures. The allocation
/// is once per `coproc` invocation — process and pipe setup dominate.
fn execute_coprocess<'a, SE: extensions::ShellExtensions>(
	coproc: &'a ast::CoprocessCommand,
	shell: &'a mut Shell<SE>,
	params: &'a ExecutionParameters,
) -> Pin<Box<dyn Future<Output = Result<ExecutionResult, error::Error>> + Send + 'a>> {
	Box::pin(async move {
...
impl Execute for ast::FunctionDefinition {
	async fn execute<SE: extensions::ShellExtensions>(
		...
		let func_name = self.fname.value.to_string();
...
#[allow(
	...
	reason = "command execution keeps shell semantics in one control-flow path"
)]
...
#[expect(
	clippy::too_many_lines,
	reason = "assignment semantics require all variable forms together"
)]
...
#[expect(
	clippy::too_many_lines,
	reason = "redirect semantics require every redirection form together"
)]
