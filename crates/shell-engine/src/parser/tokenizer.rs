...
pub enum TokenizerError {
	/// An unterminated escape sequence was encountered at the end of the input
	/// stream.
	#[error("unterminated escape sequence")]
	UnterminatedEscapeSequence,

	/// An unterminated single-quoted substring was encountered at the end of the
	/// input stream.
	#[error("unterminated single quote at {0}")]
	UnterminatedSingleQuote(SourcePosition),

	/// An unterminated ANSI C-quoted substring was encountered at the end of the
	/// input stream.
	#[error("unterminated ANSI C quote at {0}")]
	UnterminatedAnsiCQuote(SourcePosition),

	/// An unterminated double-quoted substring was encountered at the end of the
	/// input stream.
	#[error("unterminated double quote at {0}")]
	UnterminatedDoubleQuote(SourcePosition),

	/// An unterminated back-quoted substring was encountered at the end of the
	/// input stream.
	#[error("unterminated backquote near {0}")]
	UnterminatedBackquote(SourcePosition),

	/// An unterminated extended glob (extglob) pattern was encountered at the
	/// end of the input stream.
	#[error("unterminated extglob near {0}")]
	UnterminatedExtendedGlob(SourcePosition),

	/// An unterminated variable expression was encountered at the end of the
	/// input stream.
	#[error("unterminated variable expression")]
	UnterminatedVariable,

	/// An unterminated command substitiion was encountered at the end of the
	/// input stream.
	#[error("unterminated command substitution")]
	UnterminatedCommandSubstitution,

	/// An unterminated arithmetic or other expansion was encountered at the end
	/// of the input stream.
	...
	/// An unterminated here document sequence was encountered at the end of the
	/// input stream.
...
enum HereState {
	...
	CurrentTokenIsHereTag { remove_tabs: bool, operator_token_result: TokenizeResult },
...
struct CrossTokenParseState {
	...
	/// Ordered queue of here tags for which we're still looking for matching
	/// here-document bodies.
	current_here_tags:    Vec<HereTag>,
	/// Tokens already tokenized that should be used first to serve requests for
	/// tokens.
...
impl TokenParseState {
	...
	#[allow(clippy::too_many_lines, reason = "token termination handles every shell lexical state")]
	pub fn delimit_current_token(
		...
		// If we don't have anything in the token, then don't yield an empty string
		// token *unless* it's the body of a here document.
		if !self.started_token() && !matches!(reason, TokenEndReason::HereDocumentBodyEnd) {
			return Ok(Some(TokenizeResult { reason, token: None }));
		}
		...
		match current_here_state {
			HereState::NextTokenIsHereTag { remove_tabs } => {
				...
				let operator_token_result =
					TokenizeResult { reason, token: Some(self.pop(&cross_token_state.cursor)) };

				cross_token_state.here_state =
					HereState::CurrentTokenIsHereTag { remove_tabs, operator_token_result };

				return Ok(None);
			},
			HereState::CurrentTokenIsHereTag { remove_tabs, operator_token_result } => {
				if self.is_newline() {
					return Err(TokenizerError::MissingHereTag(self.current_token().to_owned()));
				}
			...
			HereState::NextLineIsHereDoc => {
				...
				if let Some(last_here_tag) = cross_token_state.current_here_tags.last_mut() {
					...
					let result = TokenizeResult { reason, token: Some(token) };
			...
			HereState::InHereDocs => {
				...
				cross_token_state
					.queued_tokens
					.push(TokenizeResult { reason, token: Some(self.pop(&cross_token_state.cursor)) });
...
pub fn uncached_tokenize_str(
	...
	loop {
		match tokenizer.next_token()? {
			TokenizeResult { token: Some(token), .. } => tokens.push(token),
			TokenizeResult { reason: TokenEndReason::EndOfInput, .. } => break,
			_ => (),
		}
	}
...
impl<'a, R: ?Sized + std::io::BufRead> Tokenizer<'a, R> {
	pub fn new(reader: &'a mut R, options: &TokenizerOptions) -> Self {
		Tokenizer {
			...
			cross_state: CrossTokenParseState {
				cursor:               SourcePosition { index: 0, line: 1, column: 1 },
	...
	#[expect(clippy::unnecessary_wraps, reason = "the tokenizer interface permits absent locations")]
	...
	/// Consumes a nested construct (e.g., `$((...))` or `$[...]`), handling
	/// nested delimiters and here-documents.
	///
	/// # Arguments
	///
	/// * `state` - The current token parse state to append characters to.
	/// * `terminating_char` - The character that terminates the construct (e.g.,
	///   `)` or `]`).
	/// * `nesting_open` - The character that increases nesting depth when
	///   encountered (e.g., `(` or `[`).
	...
	/// Returns the next token from the input stream, optionally stopping early
	/// when a specified terminating character is encountered.
	///
	/// # Arguments
	///
	/// * `terminating_char` - An optional character that, if encountered, will
	///   stop the tokenization process and return the token up to that
	///   character.
	/// * `include_space` - If true, include spaces in the tokenization process.
	///   This is not typically the case, but can be helpful when needing to
	///   preserve the original source text embedded within a command
	///   substitution or similar construct.
	#[expect(
		clippy::cognitive_complexity,
		reason = "shell tokenization has many required lexical states"
	)]
	#[expect(
		clippy::if_same_then_else,
		reason = "equivalent delimiters preserve distinct parser branches"
	)]
	#[expect(
		clippy::panic_in_result_fn,
		reason = "invalid reader state is reported through tokenization"
	)]
	#[expect(clippy::too_many_lines, reason = "shell tokenization handles the complete grammar")]
	#[allow(clippy::unwrap_in_result, reason = "validated parser invariants guarantee these values")]
	fn next_token_until(
		&mut self,
		...
		while result.is_none() {
			...
			// When we hit the end of the input, then we're done with the current token (if
			// there is one).
			if next.is_none() {
				...
				if !matches!(self.cross_state.here_state, HereState::None) {
					...
					let tag_positions = self
						...
						.join(", ");
					return Err(TokenizerError::UnterminatedHereDocuments(tag_names, tag_positions));
				}

				result =
					state.delimit_current_token(TokenEndReason::EndOfInput, &mut self.cross_state)?;
			//
			...
			//
				...
				if state.unquoted() && self.is_operator(hypothetical_token.as_ref()) {
					...
					if self.cross_state.arithmetic_expansion {
						...
						self.cross_state.here_state = HereState::NextTokenIsHereTag { remove_tabs: true };
			...
			//
			// See if this is a character that changes the current escaping/quoting
			// state.
			//
				...
				if c == '\\' {
					...
					if matches!(self.peek_char()?, Some('\n')) {
						...
						// Make sure to include neither the backslash nor the newline
						// character.
					} else {
			...
			// Handle end of single-quote, double-quote, or ANSI-C quote.
				...
				&& matches!(state.quote_mode, QuoteMode::Single(..) | QuoteMode::AnsiC(..))
				&& c == '\''
			{
			...
			//
				...
				if state.started_token() {
					result = state
						.delimit_current_token(TokenEndReason::OperatorStart, &mut self.cross_state)?;
				} else {
			...
			//
				...
				if state.started_token() {
					result = state
						.delimit_current_token(TokenEndReason::NonNewLineBlank, &mut self.cross_state)?;
				...
				&& (state.started_token() || matches!(terminating_char, Some('}')))
			{
				...
				result = state.delimit_current_token(TokenEndReason::Other, &mut self.cross_state)?;
			} else {
				// If we got here, then we don't have a token in progress and we're not starting
				// an operator. Add the character to a new token.
				self.consume_char()?;
				state.append_char(c);
			}
		}
	...
	fn remove_here_end_tag(
		...
		if let Some(current_token_without_here_tag) = state.current_token().strip_suffix(tag_str) {
				...
				|| current_token_without_here_tag.ends_with('\n')
			{
				...
				*result = state
					.delimit_current_token(TokenEndReason::HereDocumentBodyEnd, &mut self.cross_state)?;
...
impl<R: ?Sized + std::io::BufRead> Iterator for Tokenizer<'_, R> {
	...
	fn next(&mut self) -> Option<Self::Item> {
		match self.next_token() {
			#[expect(clippy::manual_map, reason = "the token payload and error must remain distinct")]
...
HERE2
	...
	fn tokenize_unterminated_backquote() {
		assert!(matches!(tokenize_str("`"), Err(TokenizerError::UnterminatedBackquote(_))));
	}
	...
	fn tokenize_unterminated_command_substitution() {
		...
		assert!(matches!(tokenize_str("$("), Err(TokenizerError::UnterminatedExpansion)));
	}
	...
	fn tokenize_unterminated_parameter_expansion() {
		assert!(matches!(tokenize_str("${x"), Err(TokenizerError::UnterminatedVariable)));
	}
