...
pub enum TildeExpr {
	...
	/// `~-`
	OldWorkingDir,
	/// Represents a tilde expansion of the form `~+N`, referring to the Nth
	/// directory in the shell's directory stack, starting at the top of the
	/// stack. Note that the directory stack is expected to contains the current
	/// working directory as its topmost entry.
	NthDirFromTopOfDirStack {
		/// Index into the directory stack (zero-based: 0 is the top of the
		/// stack).
		n:         usize,
	...
	/// Represents a tilde expansion of the form `~-N`, referring to the Nth
	/// directory in the shell's directory stack, starting at the bottom of the
	/// stack.
	NthDirFromBottomOfDirStack {
		/// Index into the directory stack (zero-based: 0 is the bottom of the
		/// stack).
		n: usize,
	},
}
...
pub enum ParameterExpr {
	...
	/// Remove the smallest suffix from the given string matching the given
	/// pattern.
	...
	/// Remove the largest suffix from the given string matching the given
	/// pattern.
	...
	/// Remove the smallest prefix from the given string matching the given
	/// pattern.
	...
	/// Remove the largest prefix from the given string matching the given
	/// pattern.
...
/// Represents a sub-word that is either a brace expression or some other word
/// text.
#[derive(Clone, Debug)]
#[cfg_attr(test, derive(PartialEq, Eq, serde::Serialize, serde::Deserialize))]
...
pub(crate) fn parse_array_assignment(
	...
	let elements_as_words = elements
		.into_iter()
		.map(|(key, value)| (key.map(|k| ast::Word::new(k.as_str())), ast::Word::new(value.as_str())))
		.collect();
...
peg::parser! {
	 grammar expansion_parser(parser_options: &ParserOptions) for str {
		// Helper rule retained to keep rule call sites aligned with upstream.
...
mod tests {
	...
	fn parse_tilde_after_colon() -> Result<()> {
		...
		assert!(matches!(parsed[0].piece, WordPiece::Text(_)));
		assert!(matches!(parsed[1].piece, WordPiece::TildeExpansion(_)));
