		...
		for (index, frame) in self.iter().enumerate() {
				...
				&& let Some(entry) = &frame.entry
			{
				...
				if let Some(entry_start) = &entry_si.start {
					write!(f, " entered at {}:{}", entry_si.source, entry_start)?;
				}
			}
...
mod tests {
	...
	fn test_call_stack_iter() {
		...
		assert!(
			matches!(&frames[0].frame_type, FrameType::Script(ScriptCall { source_info: SourceInfo { source: file_path, .. }, .. }) if file_path == "script3.sh")
		);
		assert!(
			matches!(&frames[1].frame_type, FrameType::Script(ScriptCall { source_info: SourceInfo { source: file_path, .. }, .. }) if file_path == "script2.sh")
		);
		assert!(
			matches!(&frames[2].frame_type, FrameType::Script(ScriptCall { source_info: SourceInfo { source: file_path, .. }, .. }) if file_path == "script1.sh")
		);
	}
}
