...
/// Failure to resolve a shell job specification.
#[derive(Clone, Copy, Debug, Eq, PartialEq, thiserror::Error)]
pub enum JobSpecError {
	/// No managed job matches the specification.
	#[error("no such job")]
	NotFound,
	/// More than one managed job matches the command text.
	#[error("ambiguous job spec")]
	Ambiguous,
}
...
impl JobTask {
	...
	fn detach(self) {
		match self {
			Self::External(process) => process.detach(),
			Self::Internal(handle) => drop(handle),
		}
	}

	fn preserve_on_shell_exit(&mut self) {
		if let Self::External(process) = self {
...
impl JobManager {
	...
	/// Keeps a managed job alive when the shell's job table is dropped.
	pub fn keep_on_shell_exit(&mut self, id: usize) -> bool {
		let Some(job) = self.jobs.iter_mut().find(|job| job.id == id) else {
			return false;
		};
		job.preserve_on_shell_exit();
		true
	}

	fn remove_at(&mut self, index: usize) -> Job {
	...
	fn refresh_annotations(&mut self) {
		...
		if let Some(current) = self.jobs.last_mut() {
			current.annotation = JobAnnotation::Current;
		}
		if len > 1 {
			self.jobs[len - 2].annotation = JobAnnotation::Previous;
		}
	}
	...
	/// Resolves a job specification to a mutable managed job.
	pub fn resolve_job_spec(&mut self, job_spec: &str) -> Result<&mut Job, JobSpecError> {
		match self.resolve_job_spec_selector(job_spec)? {
			JobSelector::JobId(id) => self
				.jobs
				.iter_mut()
				.find(|job| job.id == id)
				.ok_or(JobSpecError::NotFound),
			JobSelector::ProcessId(pid) => self
				.jobs
				.iter_mut()
	...
	/// Resolves a job specification to a wait selector.
	pub fn resolve_job_spec_selector(&self, job_spec: &str) -> Result<JobSelector, JobSpecError> {
		let remainder = job_spec.strip_prefix('%').ok_or(JobSpecError::NotFound)?;

		match remainder {
			"" | "%" | "+" => self
				.current_job()
				.map(|job| JobSelector::JobId(job.id))
				.ok_or(JobSpecError::NotFound),
			"-" => self
				.prev_job()
				.map(|job| JobSelector::JobId(job.id))
			...
			value if value.chars().all(char::is_numeric) => {
				let id = value.parse::<usize>().map_err(|_| JobSpecError::NotFound)?;
				self
					.jobs
					.iter()
					.any(|job| job.id == id)
			...
			value => {
				let (query, contains) = value
					.strip_prefix('?')
					.map_or((value, false), |query| (query, true));
				self.resolve_command_selector(query, contains)
			},
		}
	}
	...
	fn resolve_command_selector(
		...
		query: &str,
		contains: bool,
	) -> Result<JobSelector, JobSpecError> {
		let mut matches = self.jobs.iter().filter(|job| {
			if contains {
				job.command_line.contains(query)
			} else {
				job.command_line.starts_with(query)
			}
		});
		let id = matches
			.next()
	...
	pub fn poll(&mut self) -> Result<Vec<JobResult>, error::Error> {
		...
		while i != self.jobs.len() {
			if let Some(result) = self.jobs[i].poll_done()? {
				...
				results.push((self.remove_at(i), Ok(ExecutionResult::success())));
			} else {
				i += 1;
			}
		}
	...
	fn sweep_completed_jobs(&mut self) -> Vec<Job> {
		...
		while i != self.jobs.len() {
			if self.jobs[i].tasks.is_empty() {
				completed_jobs.push(self.remove_at(i));
			} else {
				i += 1;
			}
		}
...
impl Job {
	...
	/// Keeps every task alive when the shell's job table is dropped.
	fn preserve_on_shell_exit(&mut self) {
		for task in &mut self.tasks {
			task.preserve_on_shell_exit();
		}
	}

	/// Detaches every task so it can continue after this job leaves the table.
	fn detach(mut self) {
