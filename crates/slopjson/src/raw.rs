...
/// Magic newtype-struct name the in-crate [`Deserializer`](crate::Deserializer)
/// recognizes as a raw-capture request.
pub const TOKEN: &str = "$omp_slopjson::private::RawValue";
...
/// [`from_str`](crate::from_str) / [`parse`](crate::parse) — with one
/// exception: a bareword string value (`{"paths": packages/foo/*}`) is only
/// grammatical in object/array value position, so its captured span does not
/// stand alone as a document. Use [`repair_json`](crate::repair_json) or
/// `Value`'s `Display` when strict JSON text is required.
///
/// Capture only works with this crate's deserializer; other formats fail
...
///
/// ```
/// use omp_slopjson::{RawValue, from_str};
///
