//! Authoritative property-key vocabulary shared by prompt producers and templates.
...
// Nested field identifiers remain registry entries because template member access uses the
// same identifier grammar as top-level lookups.
...
pub const ALL: &[&str] = &[
	CWD, VCS, HOST, MODEL, REPOSITORIES, ROOTS, ADDITIONAL_ROOTS, ACTIVE_REPOSITORY, CONTEXT_FILES,
	DIRECTORY_CONTEXT, WORKSPACE_TREES, SKILLS, RULES, PERSONALITY, RENDER_MERMAID,
	INCLUDE_WORKSTATION, INCLUDE_MODEL, INCLUDE_WORKSPACE_TREE, INCLUDE_SKILLS, SECRETS_ENABLED,
	NULL_PROMPT, INTENT_FIELD, CUSTOM_PROMPT, APPEND_PROMPT, TOOL_INVENTORY, TOOLS, SCHEMES,
	SCHEME_SELECTORS, COMPUTER, DEVICE_GUIDANCE, AUTO_QA_GUIDANCE, DELEGATION, MUTATIONS,
	EDIT_HASHLINE, EDIT_APPLY_PATCH, EDIT_SLOPPY, MEMORY, AGENT_NAME,
	AGENT_DESCRIPTION, AGENT_PROMPT, SHARED_CONTEXT, PLAN_PATH, PLAN_CONTENT, WORKSPACE_ROOT,
	OUTPUT_SCHEMA, OUTPUT_SCHEMA_TS, SELF_NAME, SELF_ROLE, IRC_ENABLED, ROSTER_GENERATION, PEERS,
	CAPS, PLAN_MODE, EAGER, RETRY_COUNT, MAX_RETRIES, FROM, MESSAGE, TOOL_NAME, COUNT,
	ARGUMENTS_SUMMARY, RESULT_SUMMARY, SUMMARY, FIRST_NAME, USERNAME, ARGS, ARGUMENTS, ROOT, HEAD,
	OS, DISTRO, KERNEL, ARCH, CPU, TERMINAL, GPUS, IDENTIFIER, CODEX_TASK_POLICY, ROOT_URI,
	WORKTREE_ROOT_URI, PRIMARY_ROOT_URI, RELATIVE_ROOT, BRANCH, STAGED, UNSTAGED, UNTRACKED, REVISION, TRUNCATED,
	PRIMARY, CANONICAL_URI, GRANT_ID, PATH, ORIGIN, CONTENT, RENDERED, NAME, DESCRIPTION, READABLE,
	MINTABLE, SELECTORS, ENABLED, BATCH, CONCURRENCY, QUEUED, SCOUT_AVAILABLE, COORDINATION,
	FORMAT_ON_WRITE, FETCH, EDITOR, ESCALATION, STANDING, RECALL, ROLE, STATUS, ACTIVITY,
	CODEX_STYLE, PARALLEL_TOOL_CALLS, STRUCTURED_YIELD,
];
...
mod tests {
	...
	fn registry_is_unique_and_disjoint_from_reserved_words() {
		...
		const RESERVED: &[&str] = &[
			"if", "elif", "else", "endif", "for", "endfor", "in", "and", "or", "not", "set",
			"raw", "endraw", "true", "false", "none", "loop",
		];
