...
`@omp.tool`, the `dyn` core tool, `omp.ToolPath`, the dynamic tool policy, and
...
`dyn` catalog (`{"do_": "docs/<name>"}`) at all — that is what `RegisterTools` is for — but the
...
does not. Second, a `dyn` invoke (`{"do_": "invoke/<name>", …}`) fires exactly one `tool_call` with
...
| `allowed_devices` | `frozenset[str] \| None` | `None` | Allowlist of device names (see `docs/py/01-devices.md`) the child may reach through the `dyn` core tool. `None` inherits the parent's set. The empty frozenset yields a child with core tools only. |
...
registered with the host, discovered through `dyn` search, schema fetched on
demand through `dyn` docs, dispatched with `{"do_": "invoke/agent", …}`.
...
- **`RegisterTools` is host-facing, and `omp.agents` adds nothing to it.**
  ...
  schema, rev, and constraints to answer the `dyn` catalog at all — and never
...
- `task.md:3-21` makes `task` a registered tool with a dynamically generated
  ...
  dispatched through the `dyn` core tool (soft/hard intent, surface decided by
  ...
  documentation fetched on demand by `{"do_": "docs/agent"}`, which costs zero TTFT
  when unused.
