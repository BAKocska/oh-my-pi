...
| **Fold** | `@omp.message_renderer`, `@omp.completion` | core → host, on demand | `Tml` / `Completion` | fail-open: default rendering, empty list |
...
flowchart LR
    ...
    SCENE["crates/chat-ui<br/>slot registry, StatusFacts"]
...
1. **A rail reserves columns from the layout, it does not steal them.** The live region
   ...
   visible rails. `crates/chat-ui/src/sidebar.rs:59` already exposes exactly this as
...
columns (`Sidebar::reserved`, `crates/chat-ui/src/sidebar.rs:59`), the compositor owns
...
| `set_working_message` | CONTROL, one-way | per-turn | fail-open; built-in message restored |
...
Every symbol below is reached as `omp.ui.<name>`, except the four decorators
(`@omp.command`, `@omp.shortcut`, `@omp.completion`, `@omp.message_renderer`), which are
...
#### `ui.join(parts: Iterable[Tml], sep: Tml | str = "") -> Tml`
...
| `<diff>` | Rust component (`crates/tui/src/components/diff.rs`) with no markup tag. See §6 — adding one is proposed work. |
...
| `width` | `int \| None` | `None` | Rail width in cells, including its vertical rule. Ignored outside `SIDEBAR_*`. Defaults to 30, matching `crates/chat-ui/src/sidebar.rs:16`. |
...
built-in band already has (`crates/chat-ui/src/scene.rs:356-357`). An extension does not
...
`ui.Pct(n)` is a percentage dimension; a bare `int` is cells. `ui.Margin(top, right,
...
`ui.Field` is the Python mirror of `<field>`: `Field(id, kind, label, desc=None,
...
#### `@dataclass(frozen=True) class ui.AskAnswer`
...
#### `@dataclass(frozen=True) class ui.CompletionItem`
...
`render_result` pair anywhere in this namespace.
...
`MessageView` is a read-only projection owned by `docs/py/08-context.md`.
...
`ui.Arg(name: str, description: str = "", usage: str | None = None)`. `usage` ghosts
...
`arg_completions` supplies them dynamically: `async def f(query: ui.ArgQuery, ctx:
omp.Context) -> Iterable[CompletionItem]`, where `ui.ArgQuery` is a frozen dataclass
...
`ui.limits` holds the tunables an extension may read but not set.
...
async def paint(event, ctx: omp.Context) -> None:
    s = ctx.session.stats                     # docs/py/09-journal.md
    git = await ctx.env.git_facts()           # docs/py/11-env.md

    ui.set_status("model", ui.tml(
        "<segment fg=accent><ico:robot/> {model}</segment>", model=s.model), order=10)

    pct = 100 * s.context_tokens // max(s.context_window, 1)
    ui.set_status("ctx", ui.tml(
        "<segment fg={tone}>{pct}%</segment>",
        tone=ui.Token.ERR if pct > 90 else ui.Token.WARN if pct > 70 else ui.Token.MUTED,
        pct=pct), order=20)

    ui.set_status("cost", ui.tml(
        "<segment fg=secondary>${cost}</segment>", cost=f"{s.cost_usd:.2f}"), order=30)
    ...
    ui.mount(ui.Slot.FOOTER, ui.tml(
        ...
        "  <text fg=muted>{tokens} tok · {rate} tok/s</text>"
        "</row>",
        cwd=ctx.cwd, tokens=f"{s.total_tokens:,}", rate=f"{s.tokens_per_second:.0f}",
    ))
```
...
swallowed any key. And `buildSegmentContext`'s 300 lines are `ctx.session.stats` — the
harness already had the numbers, and 71 packages reconstructing them from event replay
...
async def rail(event, ctx: omp.Context) -> None:
    ...
    ui.mount(ui.Slot.SIDEBAR_RIGHT, ui.tml(
        ...
        "    <tr><td><text fg=muted>ctx</text></td><td><text>{pct}%</text></td></tr>"
        "  </table>"
        "  {git}"
        "  <spacer grow/>"
        "  <text dim>ctrl+alt+b toggles</text>"
        "</col>",
        title=ctx.session.title or "session",
        model=s.model,
        turns=s.turns,
        pct=100 * s.context_tokens // max(s.context_window, 1),
...
| Non-modal full-height rail with column reservation | `crates/tui/src/ui.rs:1055` `show_overlay` + `OverlayOptions::non_modal`/`fill_height`; `crates/chat-ui/src/sidebar.rs:16,18,59` (`WIDTH = 30`, `MIN_VIEWPORT = 96×20`, `reserved()`) | Yes. `Sidebar` is a working single-tenant instance of the slot design. |
...
| Status band with powerline caps | `crates/tui/src/components/status.rs`; band assembly at `crates/chat-ui/src/scene.rs:250-313` | Component yes; **assembly no** — the band is built from a closed `StatusFacts` struct. |
...
`BackendEvent::ToolOutput { chunk: Str }` (`crates/chat-ui/src/lib.rs:215,233`) become
...
A new package so `crates/tui` and `crates/chat-ui` can depend on the UI messages without
...
2. **`<diff>` markup tag.** `components::Diff` exists (`crates/tui/src/components/diff.rs`)
   with no way to reach it from markup. Add `"diff"` to `is_catalog_tag` and `build`, with
   props for the unified-diff body, `context=N`, and a colorblind-safe palette flag. This
   is the single highest-value vocabulary addition: `@heyhuynhgiabuu/pi-pretty` and
   `pi-cc-extensions` both exist substantially to render diffs, and both re-implemented it.
...
4. **`status=` alias set on `<task>`.** It already accepts `in_progress`, `completed`,
   ...
   `omp.agents.RunStatus.value` maps one-to-one and no caller needs a translation dict.
5. **`icons.tsv` rows.** Nine additions across all three tiers: `pending`, `running`,
   ...
   already exists.) These make `ui.icon(status.value)` total for both
...
6. **Band composition without a per-frame `Vec<Line>`.** N mounts in one placement must
   ...
   streams rows through `RenderedFrame { frame, stable_rows, damage: SmallVec<(u16,u16), 4> }`
   (`crates/chat-ui/src/scene.rs:35-42`). The addition is a `Bands` type that measures each
...
#### `crates/chat-ui` — the slot registry and the input adapters
...
| `ctx.ui.setTheme` / `getAllThemes` / `getTheme` | **Dropped** | An extension changing the user's theme is hostile, and every reason to want it (a badge color, a syntax sample) is served by a CSS color on one element. |
| `getToolsExpanded` / `setToolsExpanded` | **Dropped** | A user display preference. An extension reads `RenderCtx.collapsed` and renders accordingly; it does not get to flip the preference. |
...
| Handlebars command templating (`{{args}}`, `{{#list}}`, `{{#table}}`, `{{#codeblock}}`) | **Not provided to Python** | A second template language inside the command layer, for a language that already has f-strings. File-based Markdown commands keep it, because they have no other way to compute; a `@omp.command` returns `Prompt(text)` it built in Python. Conflicts with `.plan/feature-map/slash-commands.md`, which documents the full Handlebars set — see §6.6. |
...
| Precedent | `crates/chat-ui/src/sidebar.rs` is exactly this, working | None |
...
1. `FEATURES.md:1056` — *"extension UI bridging: dialogs, notify, status/widget/title,
   ...
   should be split.
2. `slash-commands.md` documents the full Handlebars set (`{{args}}`, `{{ARGUMENTS}}`,
   `{{arg 1}}`, `{{#list}}`, `{{join}}`, `{{when}}`, `{{#table}}`, `{{#codeblock}}`,
   `{{#xml}}`). §6.4 does not offer it to Python. Resolution: the templating layer stays
   scoped to file-based Markdown commands, which have no other way to compute. Making it
   available to `@omp.command` would mean maintaining a template engine for the benefit of
   code that can already interpolate.
3. `interactive-ui.md` M2 lists *"50+ slot JSON themes"* with `/theme` live preview.
   §6.4 drops extension theme control. No functional conflict — the theme system is a user
   surface — but an extension shipping a theme is a plausible want, and it should arrive as
   a *manifest asset* the theme loader picks up, not as a `ui.set_theme` call. Out of scope
   here; belongs with `docs/py/14-deploy.md`.
