...
A prelude helper is **not a tool**. It is not inserted into the tool `Registry`, advertised to the model as a tool schema, included in the tool prompt, available as `tool.<name>`, or routed through `dyn`. Use it for an operation that should read naturally inside Python eval code and whose arguments and result have a JSON representation.

Packaging, installation, activation, and the rest of the manifest belong to [14-deploy.md](14-deploy.md). Tool and `dyn` semantics belong to [01-devices.md](01-devices.md).
