...
Notice what is missing: there is no per-extension *tool slot*. **Extensions register with the host, never with the model.** The host must know a tool's name, family, rev, schema, and constraints — that is what `RegisterTools` is for, and it is what makes the catalog answerable at all. What never happens is unbounded registration with the *model*: no schema slot per extension, no tool-array growth, no `setActiveTools`, no `loadMode`. On Codex, TTFT scales roughly 1:1 with registered tool count because every schema feeds the sampler's grammar — 40 dormant MCP endpoints tax every token of every turn. Extension capabilities ride the catalog instead, reached through the one permanently stable `dyn` core tool ([01-devices.md](01-devices.md) owns it, the `do_` grammar, and `omp.ToolPath`): `{"do_": "search"}` lists, `{"do_": "docs/<name>"}` fetches full docs and schema on demand, and `{"do_": "invoke/<name>", ...}` dispatches. Which declarations also occupy model-facing slots is the user's call, not the extension's: the **dynamic tool policy** (`tools.policy`, [01-devices.md](01-devices.md)) resolves each declaration's soft/hard *intent* into a surface. Under the default `auto` policy exactly two sanctioned exceptions to slot growth exist, both bounded: the single stable `dyn` slot itself, and **hard tools** — `@omp.tool(kind="hard")` declarations granted a budgeted, audited model-facing slot each while remaining in the `dyn` tree, addressable by `omp.ToolPath`. Under `tool_only` the user has explicitly bought slot growth wholesale — every declaration surfaces as a slot and the `dyn` tool is not advertised at all; `device_only` demotes even hard intent to a device. Availability changes append one system-notification item; the request's tool array is byte-identical before and after, so the prompt prefix cache survives. (Rev 2 of this passage dispatched and read docs through the retired read/write device URL scheme; the Rev 2.1 rulings replaced that scheme with the `dyn` ops entirely — the reversal is recorded in *Revision 2.1* below and at the transport's owner.)
...
|---|---|---|
| static, no Python | served from the manifest alone; the child never boots for it | the `dyn` search listing; the subscription mask; command and completion *names* |
...
class ToolEntry:
    ...
    summary: str                                # one line, shown by dyn search
...
- **One event, one chain.** `tool_call` fires exactly once per logical dispatch, carrying a tagged `target` — core tool, device, or MCP endpoint ([05-hooks.md](05-hooks.md)). A `dyn` invoke (`{"do_": "invoke/shell_exec", ...}`) does *not* fire a `dyn` gate and then a device gate; it fires one `tool_call` with the RESOLVED `target=DeviceCall(...)` and **decoded** args — `dyn` is transport, never the policy subject, so a guard on the resolved device cannot be bypassed by the envelope. The catalog reads, `search` and `docs`, fire `tool_call` with `target=CoreTool("dyn")`. A policy author cannot accidentally guard `bash` while waving through the device that does the same thing, and cannot double-prompt the user for one action.
...
- **Zero schema slots.** The model sees `computer` in the catalog (`{"do_": "search"}`), pulls the full schema with `{"do_": "docs/computer"}` only in the turn it needs it, and dispatches with `{"do_": "invoke/computer", ...}` on the `dyn` core tool. Malformed args echo the schema back so the model self-corrects in place ([01-devices.md](01-devices.md)).
