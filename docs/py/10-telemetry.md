...
filed through the `report_issue` device. False positives are **explicitly welcome** — a false
positive is training data for the detector, and a report costs one line.

```mermaid
flowchart TD
    A["standing instruction<br/>in system prompt"] --> B["model notices<br/>result ≠ docs"]
    B --> C["dyn invoke report_issue"]
...
| `@omp.device`, `@omp.tool`, `omp.devices`, `omp.mcp`, the `dyn` core tool and its `do_` grammar, `omp.ToolPath`, the dynamic tool policy | `docs/py/01-devices.md` |
...
- `devices: tuple[str, ...]` — extension and MCP device wire names mounted at start, reachable
  through the `dyn` catalog (`docs/py/01-devices.md`).
...
- `decoded_args: object | None` — the arguments as the executor received them, after charitable
  ...
  are the decoded args, never the `do_` envelope that carried them, exactly as at the gate.
...
host can answer the `dyn` catalog (`{"do_": "docs/<name>"}`) at all, while the model's tool array never grows
(`docs/py/01-devices.md`). These events record how the harness spent a *provider* constraint
...
Files an AutoQA report from extension code, alongside the ones the model files through the
`report_issue` device. Returns the issue identifier.
...
Wiring that into the loop closes it. The model files reports through the `report_issue` device under
the standing instruction; an extension triages them against per-rev metrics and folds the obvious
ones:
