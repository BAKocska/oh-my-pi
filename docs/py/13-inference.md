...
tool policy an extension's capability reaches the model through the `dyn` core tool:
`{"do_": "docs/<name>"}` fetches docs and schema on demand, `{"do_": "search"}` discovers, and
`{"do_": "invoke/<name>", …}` dispatches, with the arguments flattened into the `do_` envelope. A
device therefore has **no schema in the request**, and its arguments arrive inside `dyn`'s one
permanently stable envelope, re-nested against the device's own schema at `ARGS_FINALIZED`.
...
3. **`crates/proto`** — extend the contracts that exist; do not open a parallel family. Every proposal
   ...
   **`omp/toolhost/v1/toolhost.proto` — three additive changes.**
   ...
   `RegisterTools` is host-facing, which is precisely why the host can answer the `dyn` catalog at all.
...
separate accessor for the host-facing view that the `dyn` catalog serves from, since the host must still know
