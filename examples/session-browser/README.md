...
- Frozen export defect: `omp.SessionKind` is named in `crates/py/python/omp/__init__.py:952-955` and documented by `docs/py/09-journal.md` §“omp.SessionInfo”, but `SessionKind` is missing from the actual import list at `crates/py/python/omp/__init__.py:335-347`. The example uses the real `omp.sessions.SessionKind` owner instead.
