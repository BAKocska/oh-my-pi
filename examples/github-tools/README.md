...
One soft `github` device replaces the five-plus per-operation schema slots. Its typed `path` dispatch covers `pr/list`, `pr/view`, `ci/runs`, `issue/list`, and `release/list`; `pr/comment` and `issue/comment` are declared mutation paths. The device is reached only through `dyn`, retains the complete redacted JSON response in its typed verdict, projects at most twenty rows through `omp.Budget`, and surfaces GitHub rate-limit headers as a `RateLimit` field (`docs/py/01-devices.md` §§“The three ops, end to end” and “`@omp.device`”; `docs/py/02-verdicts.md` §§“`omp.Payload`” and “Projecting for the model”).
