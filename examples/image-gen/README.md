...
- `omp.ProviderHandle.request` is missing. The frozen handle exposes only `retract`, `replace`, `models`, and `is_authenticated` at `crates/py/python/omp/provider.py:811-870`; `docs/py/13-inference.md:96-110` says `GENERATE_IMAGE` uses the shared provider machinery but defines no extension-visible request/typed response call. Consequently `_request_image` names the required seam and fails with `NotWiredError` on the frozen layer; the stub smoke replaces only that boundary. The missing contract must include returned media bytes and the per-call usage receipt so cost is not estimated in Python.
